// ==========================================
// File: lib/features/dashboard/teacher/pages/teacher_publish_results.dart
// ==========================================
...
class _TeacherPublishResultsState extends State<TeacherPublishResults> {
  ...
  List<Map<String, dynamic>> get _gradingSystem =>
      GradingUtils.getGradingSystemForTier(_classTier, _provider.schoolSettings ?? {});

  bool get _showConduct => (_provider.schoolSettings?['show_conduct'] ?? true) == true;
  bool get _showTeacherComment => (_provider.schoolSettings?['show_teacher_comment'] ?? true) == true;
  bool get _showAttendance => (_provider.schoolSettings?['show_attendance_summary'] ?? true) == true;
  ...
  Map<String, dynamic>? _behavioralFor(String? studentId) {
    if (studentId == null) return null;
    try {
      return _behavioralRatings.firstWhere((b) => b['student_id'] == studentId);
    } catch (_) {
      return null;
    }
  }
  ...
  Future<void> _computeSummaries() async {
    ...
    try {
      ...
      for (final student in _students) {
        ...
        final studentScores = _scores.where((s) => s['student_id'] == sid).toList();
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const Text('Incomplete Scores', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700, color: Color(0xFF111827))),
  ...
  Future<void> _saveBehavioralRatings({
    ...
    if (existing != null) {
      ...
      await _supabase.from('student_behavioural_ratings').insert({
        'school_id': _provider.schoolId,
        'student_id': studentId,
        'class_id': _classId,
        'session_id': _sessionId,
  ...
  void _showEditSheet(Map<String, dynamic> student) {
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => _TeacherStudentEditSheet(
        studentName: name,
        initialData: data,
        behavioralRatings: data.containsKey('behavioral_ratings') ? Map<String, String>.from(data['behavioral_ratings'] as Map? ?? {}) : <String, String>{},
        ...
        child: Column(
          ...
          children: [
            Container(
              ...
              child: Icon(Icons.supervisor_account_outlined, size: 36, color: Colors.grey.shade400),
            ),
            const SizedBox(height: 16),
            Text('No form class assigned', style: TextStyle(color: Colors.grey.shade700, fontSize: 15, fontWeight: FontWeight.w500)),
          ],
        ),
      );
    }
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                const Text('Publish Results', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w700, color: Color(0xFF111827), letterSpacing: -0.5)),
                const Spacer(),
                Container(
                  ...
                  child: Row(
                    mainAxisSize: MainAxisSize.min,
                    children: [
                      const Icon(Icons.class_rounded, size: 14, color: Color(0xFF00695C)),
                      const SizedBox(width: 4),
                      Text(_className, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: Color(0xFF00695C))),
                    ],
                  ),
                ),
                const SizedBox(width: 12),
            ...
            Text('$sessionName \u2014 $termName', style: const TextStyle(fontSize: 13, color: Colors.grey)),
            const SizedBox(height: 24),
            ...
            if (_students.isNotEmpty) ...[
                ...
                children: [
                  Expanded(
                    child: SizedBox(
                      ...
                      child: ElevatedButton.icon(
                        ...
                        label: Text(_isComputing ? 'Computing...' : 'Compute Summaries', style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600)),
                        style: ElevatedButton.styleFrom(
                          backgroundColor: const Color(0xFFE65100),
                          foregroundColor: Colors.white,
                  ...
                  Expanded(
                    child: SizedBox(
                      height: 44,
                      child: ElevatedButton.icon(
                        onPressed: (_isPublishing || computed == 0) ? null : _publishAll,
                        icon: const Icon(Icons.check_circle, size: 18),
                        label: Text('Publish ($published/$computed)', style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600)),
                        style: ElevatedButton.styleFrom(
                  ...
                  Expanded(
                    child: SizedBox(
                      ...
                      child: OutlinedButton.icon(
                        onPressed: (_isPublishing || published == 0) ? null : _unpublishAll,
                        icon: const Icon(Icons.undo, size: 18),
                        label: const Text('Unpublish', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: Color(0xFF6B7280))),
                        style: OutlinedButton.styleFrom(
                          side: BorderSide(color: Colors.grey.shade300),
                          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8)),
                        ),
              ...
              Container(
                ...
                child: Row(
                  children: [
                    Icon(Icons.info_outline_rounded, size: 16, color: const Color(0xFF1A237E).withOpacity(0.6)),
                    const SizedBox(width: 8),
                    Expanded(
                      child: Text(
                        '${_students.length} students  \u2022  $computed computed  \u2022  $published published  \u2022  Tier: $_classTier',
                        style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: const Color(0xFF1A237E).withOpacity(0.7)),
                      ),
                    ),
                  ],
                ),
                    ...
                    Container(
                      ...
                      child: const Row(
                        children: [
                          ...
                          SizedBox(width: 150, child: Text('Student', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w700, fontSize: 11))),
                          SizedBox(width: 52, child: Text('Total', textAlign: TextAlign.center, style: TextStyle(color: Colors.white, fontWeight: FontWeight.w700, fontSize: 11))),
                          SizedBox(width: 52, child: Text('Avg', textAlign: TextAlign.center, style: TextStyle(color: Colors.white, fontWeight: FontWeight.w700, fontSize: 11))),
                          SizedBox(width: 48, child: Text('Grade', textAlign: TextAlign.center, style: TextStyle(color: Colors.white, fontWeight: FontWeight.w700, fontSize: 11))),
                          SizedBox(width: 52, child: Text('Pos', textAlign: TextAlign.center, style: TextStyle(color: Colors.white, fontWeight: FontWeight.w700, fontSize: 11))),
                          SizedBox(width: 80, child: Text('Days', textAlign: TextAlign.center, style: TextStyle(color: Colors.white, fontWeight: FontWeight.w700, fontSize: 11))),
                          SizedBox(width: 80, child: Text('Status', textAlign: TextAlign.center, style: TextStyle(color: Colors.white, fontWeight: FontWeight.w700, fontSize: 11))),
                          SizedBox(width: 36),
                        ],
                      ),
                    ),
                    ...
                    Container(
                      ...
                      child: Column(
                        children: _students.asMap().entries.map((entry) {
                          ...
                          final total = (summary?['total_score'] as num?)?.toInt() ?? 0;
                          ...
                          final daysP = (summary?['days_present'] as num?)?.toInt() ?? 0;
                          final daysA = (summary?['days_absent'] as num?)?.toInt() ?? 0;
                          final bgColor = entry.key % 2 == 0 ? Colors.white : const Color(0xFFFAFBFC);

                          return Container(
                            color: bgColor,
                            child: Row(
                              children: [
                                SizedBox(width: 36, child: Text('${entry.key + 1}', textAlign: TextAlign.center, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: Color(0xFF4B5563)))),
                                SizedBox(
                                  width: 150,
                                  child: Padding(
                                    padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 10),
                                    child: Text(_sName(s), style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: Color(0xFF1B2A4A)), overflow: TextOverflow.ellipsis),
                                  ),
                                ),
                                SizedBox(width: 52, child: Text(summary != null ? '$total' : '\u2014', textAlign: TextAlign.center, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: summary != null ? const Color(0xFF111827) : Colors.grey.shade400))),
                                SizedBox(width: 52, child: Text(summary != null ? avg.toStringAsFixed(1) : '\u2014', textAlign: TextAlign.center, style: TextStyle(fontSize: 11, color: summary != null ? const Color(0xFF111827) : Colors.grey.shade400))),
                                SizedBox(
                                  width: 48,
                                  child: Center(
                                    child: grade.isNotEmpty
                                        ? Container(
                                            ...
                                            decoration: BoxDecoration(
                                              color: GradingUtils.isPassingGrade(grade, _gradingSystem) ? const Color(0xFFDCFCE7) : const Color(0xFFFEE2E2),
                                              borderRadius: BorderRadius.circular(6),
                                            ),
                                            child: Text(grade, textAlign: TextAlign.center, style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: GradingUtils.isPassingGrade(grade, _gradingSystem) ? const Color(0xFF166534) : const Color(0xFF991B1B))),
                                          )
                                        : const Text('\u2014', style: TextStyle(color: Color(0xFF9CA3AF))),
                                  ),
                                ),
                                SizedBox(width: 52, child: Text(pos != null ? '$pos/$posOut' : '\u2014', textAlign: TextAlign.center, style: TextStyle(fontSize: 11, color: pos != null ? const Color(0xFF111827) : Colors.grey.shade400))),
                                SizedBox(width: 80, child: Text(summary != null ? 'P:$daysP  A:$daysA' : '\u2014', textAlign: TextAlign.center, style: TextStyle(fontSize: 10, color: summary != null ? const Color(0xFF111827) : Colors.grey.shade400))),
                                SizedBox(width: 80, child: Center(child: _buildStatusBadge(status))),
                                SizedBox(
                                  width: 36,
                                  height: 36,
                                  child: IconButton(
                                    icon: const Icon(Icons.edit, size: 15, color: Color(0xFF1A237E)),
              ...
              Center(
                child: Padding(
                  ...
                  child: Column(
                    children: [
                      Container(width: 72, height: 72, decoration: BoxDecoration(color: Colors.grey.shade100, borderRadius: BorderRadius.circular(18)), child: Icon(Icons.people_outline, size: 36, color: Colors.grey.shade400)),
                      const SizedBox(height: 16),
                      Text('No students in this class', style: TextStyle(color: Colors.grey.shade700, fontSize: 15, fontWeight: FontWeight.w500)),
                    ],
                  ),
                ),
              ),
        ...
        return Container(
          ...
          child: const Row(
            ...
            children: [
              ...
              SizedBox(width: 4),
              Text('Published', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w700, color: Color(0xFF2E7D32))),
            ],
          ),
        );
      case 'draft':
        return Container(
          ...
          child: const Row(
            mainAxisSize: MainAxisSize.min,
            children: [
              Icon(Icons.edit_note_rounded, size: 12, color: Color(0xFFE65100)),
              SizedBox(width: 4),
              Text('Draft', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w700, color: Color(0xFFE65100))),
            ],
          ),
        );
      default:
        return Container(
          ...
          child: Text('Not Set', style: TextStyle(fontSize: 10, color: Colors.grey.shade500)),
        );
    }
  }
}
...
class _TeacherStudentEditSheet extends StatefulWidget {
  ...
  @override
  State<_TeacherStudentEditSheet> createState() => _TeacherStudentEditSheetState();
}

class _TeacherStudentEditSheetState extends State<_TeacherStudentEditSheet> {
  ...
  void initState() {
    super.initState();
    _teacherCommentCtrl = TextEditingController(text: (widget.initialData['teacher_comment'] ?? '').toString());
    _daysPresentCtrl = TextEditingController(text: (widget.initialData['days_present'] ?? 0).toString());
    _daysAbsentCtrl = TextEditingController(text: (widget.initialData['days_absent'] ?? 0).toString());
  ...
  Widget _inputField(TextEditingController controller, String label, {TextInputType? keyboardType, int? maxLines}) {
    return TextField(
  ...
  Widget _ratingDropdown(String label, String value, Function(String) onChanged) {
    return DropdownButtonFormField<String>(
      value: GradingUtils.defaultBehavioralOptions.any((o) => o['value'] == value) ? value : 'Good',
      decoration: InputDecoration(
        labelText: label,
        labelStyle: TextStyle(color: Colors.grey.shade600, fontSize: 13),
        border: OutlineInputBorder(borderRadius: BorderRadius.circular(8), borderSide: BorderSide(color: Colors.grey.shade300)),
      ...
      items: GradingUtils.defaultBehavioralOptions.map((o) => DropdownMenuItem(value: o['value'] as String, child: Text(o['label'] as String, style: const TextStyle(fontSize: 13)))).toList(),
      onChanged: (v) {
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              Row(
                children: [
                  Container(width: 40, height: 40, decoration: BoxDecoration(color: const Color(0xFFF0F4FF), borderRadius: BorderRadius.circular(10)), child: const Icon(Icons.person_rounded, size: 20, color: Color(0xFF1A237E))),
                  const SizedBox(width: 12),
                  Expanded(child: Text(widget.studentName, style: const TextStyle(fontSize: 17, fontWeight: FontWeight.w700, color: Color(0xFF111827)), overflow: TextOverflow.ellipsis)),
                  InkWell(
                    borderRadius: BorderRadius.circular(8),
                    onTap: () => Navigator.pop(context),
                    child: Container(width: 32, height: 32, decoration: BoxDecoration(color: Colors.grey.shade100, borderRadius: BorderRadius.circular(8)), child: Icon(Icons.close, size: 16, color: Colors.grey.shade600)),
                  ),
                ],
              ),
              ...
              if (widget.showAttendance) ...[
                _sectionTitle('Attendance', Icons.calendar_today_rounded, const Color(0xFFE65100)),
                Row(
                  children: [
                    Expanded(child: _inputField(_daysPresentCtrl, 'Days Present', keyboardType: TextInputType.number)),
                    const SizedBox(width: 12),
                    Expanded(child: _inputField(_daysAbsentCtrl, 'Days Absent', keyboardType: TextInputType.number)),
                  ],
                ),
                const SizedBox(height: 20),
              ],
              ...
              if (widget.showConduct) ...[
                _sectionTitle('Behavioral Ratings (Nigerian Standard)', Icons.star_rounded, const Color(0xFF2E7D32)),
                Container(
                  padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8),
                  decoration: BoxDecoration(color: const Color(0xFF2E7D32).withOpacity(0.05), borderRadius: BorderRadius.circular(8), border: Border.all(color: const Color(0xFF2E7D32).withOpacity(0.1))),
                  child: Row(
                    children: [
                      Icon(Icons.info_outline_rounded, size: 14, color: const Color(0xFF2E7D32).withOpacity(0.6)),
                      const SizedBox(width: 8),
                      Expanded(child: Text('Rate each trait: Excellent / Very Good / Good / Fair / Poor', style: TextStyle(fontSize: 11, color: const Color(0xFF2E7D32).withOpacity(0.7)))),
                    ],
                  ),
                ),
                const SizedBox(height: 12),
                ...GradingUtils.behavioralFieldKeys.map((key) => Padding(
                      padding: const EdgeInsets.only(bottom: 8),
                      child: _ratingDropdown(GradingUtils.getBehavioralFieldLabel(key), _ratings[key] ?? 'Good', (v) => setState(() => _ratings[key] = v)),
                    )),
                const SizedBox(height: 20),
              ],

              if (widget.showTeacherComment) ...[
                _sectionTitle('Teacher Comment', Icons.chat_bubble_outline_rounded, const Color(0xFF1A237E)),
