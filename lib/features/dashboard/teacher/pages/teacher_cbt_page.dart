...
class _TeacherCbtPageState extends State<TeacherCbtPage> {
  ...
  Future<void> _loadQuestions(String examId) async {
    setState(() => _loadingExamIds.add(examId));
    try {
      ...
      if (mounted) setState(() => _loadingExamIds.remove(examId));
    }
  }
        ...
        builder: (ctx, setSt) => AlertDialog(
          ...
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                TextField(controller: qCtrl, maxLines: 3, decoration: const InputDecoration(labelText: 'Question Text *', border: OutlineInputBorder(), alignLabelWithHint: true)),
                const SizedBox(height: 12),
                TextField(controller: optA, decoration: const InputDecoration(labelText: 'Option A *', border: OutlineInputBorder())),
                const SizedBox(height: 8),
                TextField(controller: optB, decoration: const InputDecoration(labelText: 'Option B *', border: OutlineInputBorder())),
                const SizedBox(height: 8),
                TextField(controller: optC, decoration: const InputDecoration(labelText: 'Option C *', border: OutlineInputBorder())),
                const SizedBox(height: 8),
                TextField(controller: optD, decoration: const InputDecoration(labelText: 'Option D *', border: OutlineInputBorder())),
                const SizedBox(height: 12),
  ...
  void _showEditQuestionDialog(String examId, Map<String, dynamic> question) {
    final qCtrl = TextEditingController(text: question['question_text']?.toString() ?? '');
    final optA = TextEditingController(text: question['option_a']?.toString() ?? '');
    final optB = TextEditingController(text: question['option_b']?.toString() ?? '');
    final optC = TextEditingController(text: question['option_c']?.toString() ?? '');
    final optD = TextEditingController(text: question['option_d']?.toString() ?? '');
      ...
      for (final line in lines) {
        ...
        if (stripped.isNotEmpty) {
          ...
          if (firstChar == 'A' || firstChar == 'B' || firstChar == 'C' || firstChar == 'D') {
            ...
            final afterLetter = trimmed.replaceFirst(RegExp(r'[^a-zA-Z]*[a-d][^a-zA-Z0-9]*', caseSensitive: false), '');
        ...
        final ansMatch = RegExp(r'^(ans(?:wer)?\s*[\:\.]\s*)([a-d])', caseSensitive: false).firstMatch(trimmed);
        if (ansMatch != null) { correctOption = ansMatch.group(2)!.toLowerCase(); continue; }
  ...
  void _showBulkImportDialog(String examId) {
    ...
    showDialog(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setSt) => AlertDialog(
          title: Text(step == 0 ? 'Bulk Import Questions' : 'Preview (${parsed.length} questions)'),
          content: SizedBox(
            width: 520,
            child: step == 0
          ...
          actions: [
            if (step == 1) TextButton(onPressed: () => setSt(() => step = 0), child: const Text('Back')),
            TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('Cancel')),
            if (step == 0)
              ElevatedButton(onPressed: () {
                final marksVal = int.tryParse(marksCtrl.text.trim()) ?? 1;
              ...
              ElevatedButton(onPressed: () { Navigator.pop(ctx); _p.bulkImportQuestions(examId, parsed).then((ok) { if (ok) _loadQuestions(examId); }); }, style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF2E7D32), foregroundColor: Colors.white), child: const Text('Import All')),
          ],
        ),
      ),
    );
  ...
  Widget _optionChip(String label, String? val, String correctOpt) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(color: isSelected ? const Color(0xFFE8F5E9) : const Color(0xFFF5F5F5), borderRadius: BorderRadius.circular(6), border: Border.all(color: isSelected ? const Color(0xFF2E7D32) : Colors.grey.shade300)),
      child: Text(val != null && val.isNotEmpty ? '$label: $val' : label, style: TextStyle(fontSize: 12, color: isSelected ? const Color(0xFF2E7D32) : Colors.grey.shade700)),
    );
  }
  ...
  void initState() {
    super.initState();
    WidgetsBinding.instance.addPostFrameCallback((_) { _p.loadMyCbtExams(); });
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (exams.isEmpty) {
      body = Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        ...
        const Text('No CBT exams created yet', style: TextStyle(fontSize: 16, color: Colors.grey)),
        const SizedBox(height: 8),
        Text('Tap the + button to create one', style: TextStyle(fontSize: 13, color: Colors.grey.shade500)),
      ]));
    } else {
      body = ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          final isLoading = _loadingExamIds.contains(examId);
          final questions = _questions[examId] ?? [];
          final subjectName = e['subjects'] is Map<String, dynamic> ? (e['subjects'] as Map)['name']?.toString() ?? 'Unknown' : 'Unknown';
          ...
          return Column(mainAxisSize: MainAxisSize.min, children: [
            Container(
              ...
              decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(12), border: Border.all(color: isActive ? const Color(0xFF4CAF50) : Colors.grey.shade200, width: isActive ? 2 : 1)),
              child: InkWell(onTap: () => _toggleExpand(examId), child: Row(children: [
                ...
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text(e['title'] ?? 'CBT Exam', style: const TextStyle(fontWeight: FontWeight.bold, color: Color(0xFF1B2A4A), fontSize: 15)),
                  const SizedBox(height: 2),
                  Text('$className${classSection.isNotEmpty ? ' $classSection' : ''} \u2022 $subjectName', style: const TextStyle(fontSize: 12, color: Colors.grey)),
                  const SizedBox(height: 2),
                  Row(children: [
                ...
                const SizedBox(width: 12),
                IconButton(icon: const Icon(Icons.delete_outline, size: 20, color: Colors.red), onPressed: () => _showDeleteConfirm(examId, e['title'])),
              ])),
            ),
            if (isExpanded) ...[
              Padding(padding: const EdgeInsets.fromLTRB(24, 0, 24, 8), child: Row(children: [
                Text('Questions (${questions.length}) \u2022 Total: $totalMarks marks', style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: Color(0xFF1B2A4A))),
                const Spacer(),
                TextButton.icon(onPressed: isLoading ? null : () => _showBulkImportDialog(examId), icon: const Icon(Icons.paste, size: 18), label: const Text('Bulk Import'), style: TextButton.styleFrom(foregroundColor: const Color(0xFF0D47A1))),
                const SizedBox(width: 8),
                SizedBox(height: 36, child: ElevatedButton.icon(onPressed: isLoading ? null : () => _showAddQuestionDialog(examId), icon: const Icon(Icons.add, color: Colors.white), label: const Text('Add Question'), style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF0D47A1), foregroundColor: Colors.white, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8))))),
              ])),
              if (isLoading)
                const Padding(padding: EdgeInsets.all(40), child: Center(child: CircularProgressIndicator(color: Color(0xFF0D47A1))))
              else if (questions.isEmpty)
                Padding(padding: const EdgeInsets.all(40), child: Center(child: Column(children: [Icon(Icons.help_outline, size: 48, color: Colors.grey.shade300), const SizedBox(height: 12), Text('No questions yet', style: TextStyle(fontSize: 14, color: Colors.grey.shade500)), const SizedBox(height: 4), Text('Use "Bulk Import" to paste many questions at once', style: TextStyle(fontSize: 12, color: Colors.grey.shade400))])))
              else
                ...questions.asMap().entries.map((entry) {
                  final qi = entry.key; final q = entry.value;
                  ...
                  return Container(margin: const EdgeInsets.fromLTRB(24, 0, 24, 8), padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(10), border: Border.all(color: Colors.grey.shade100)), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      ...
                      Expanded(child: Text(q['question_text'] ?? '', style: const TextStyle(fontSize: 13, color: Color(0xFF1B2A4A)), maxLines: 3)),
                      const SizedBox(width: 4),
                      IconButton(icon: const Icon(Icons.edit_outlined, size: 18, color: Color(0xFF0D47A1)), onPressed: () => _showEditQuestionDialog(examId, q), constraints: const BoxConstraints(minWidth: 32, minHeight: 32), padding: EdgeInsets.zero, tooltip: 'Edit question'),
                      const SizedBox(width: 2),
                      IconButton(icon: const Icon(Icons.delete_outline, size: 18, color: Colors.red), onPressed: () => _showDeleteQuestionConfirm(qId, examId, q['question_text']), constraints: const BoxConstraints(minWidth: 32, minHeight: 32), padding: EdgeInsets.zero, tooltip: 'Delete question'),
                    ]),
                    const SizedBox(height: 10),
                    Wrap(spacing: 8, runSpacing: 6, children: [_optionChip('A', q['option_a']?.toString(), correctOpt), _optionChip('B', q['option_b']?.toString(), correctOpt), _optionChip('C', q['option_c']?.toString(), correctOpt), _optionChip('D', q['option_d']?.toString(), correctOpt)]),
                    const SizedBox(height: 8),
                    Row(children: [
    ...
    return Stack(children: [
      body,
      Positioned(bottom: 30, right: 30, child: FloatingActionButton.extended(onPressed: () => _showAddExamDialog(), backgroundColor: const Color(0xFF0D47A1), icon: const Icon(Icons.add, color: Colors.white), label: const Text('Create CBT', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w600)))),
    ]);
  }
  ...
  void _showAddExamDialog() {
    ...
    if (assignments.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('No subject assignments found. Contact your admin.')));
      return;
    }
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (context, setSt) => AlertDialog(
          ...
          content: SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            const SizedBox(height: 16),
            DropdownButtonFormField<String>(value: selectedAssignmentId, decoration: const InputDecoration(labelText: 'Class & Subject *', border: OutlineInputBorder()), items: assignments.map((a) {
              final subj = a['subjects'] as Map<String, dynamic>? ?? {};
              final cls = a['classes'] as Map<String, dynamic>? ?? {};
              final label = '${cls['name'] ?? ''} ${cls['section'] ?? ''} \u2022 ${subj['name'] ?? ''}'.trim();
              return DropdownMenuItem(value: a['id']?.toString(), child: Text(label, overflow: TextOverflow.ellipsis));
            }).toList(), onChanged: (v) => setSt(() => selectedAssignmentId = v)),
            const SizedBox(height: 12),
            Row(children: [
              Expanded(child: TextField(controller: durCtrl, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Duration (min)', border: OutlineInputBorder()))),
              const SizedBox(width: 12),
              Expanded(child: TextField(controller: tqCtrl, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Total Questions', border: OutlineInputBorder()))),
            ]),
          ])),
          actions: [
            TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('Cancel')),
            ElevatedButton(onPressed: () {
              if (titleCtrl.text.trim().isEmpty || selectedAssignmentId == null) return;
              final assignment = assignments.firstWhere((a) => a['id']?.toString() == selectedAssignmentId, orElse: () => {});
              if (assignment.isEmpty) return;
              _p.createCbtExam(title: titleCtrl.text.trim(), subjectId: assignment['subject_id']?.toString() ?? '', classId: assignment['class_id']?.toString() ?? '', durationMinutes: int.tryParse(durCtrl.text.trim()) ?? 60, totalQuestions: int.tryParse(tqCtrl.text.trim()) ?? 50).then((_) => Navigator.pop(ctx));
            }, style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF0D47A1), foregroundColor: Colors.white), child: const Text('Create')),
          ],
        ),
      ),
    );
  ...
  void _showDeleteConfirm(String id, String? title) {
    showDialog(context: context, builder: (ctx) => AlertDialog(shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)), title: const Row(children: [Icon(Icons.delete_outline, color: Colors.red, size: 24), SizedBox(width: 10), Text('Delete Exam')]), content: Text('Delete "${title ?? 'this exam'}"? This will also delete all its questions.'), actions: [
      TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('Cancel')),
      ElevatedButton(onPressed: () { Navigator.pop(ctx); _p.deleteCbtExam(id); }, style: ElevatedButton.styleFrom(backgroundColor: Colors.red, foregroundColor: Colors.white), child: const Text('Delete')),
    ]));
  }

  void _showDeleteQuestionConfirm(String qId, String examId, String? questionText) {
    final preview = questionText != null && questionText.length > 50 ? '${questionText.substring(0, 50)}...' : questionText ?? 'this question';
    showDialog(context: context, builder: (ctx) => AlertDialog(shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)), title: const Row(children: [Icon(Icons.delete_outline, color: Colors.red, size: 24), SizedBox(width: 10), Text('Delete Question')]), content: Text('Delete "$preview"?'), actions: [
      TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('Cancel')),
      ElevatedButton(onPressed: () { Navigator.pop(ctx); _p.deleteQuestion(qId).then((ok) { if (ok) _loadQuestions(examId); }); }, style: ElevatedButton.styleFrom(backgroundColor: Colors.red, foregroundColor: Colors.white), child: const Text('Delete')),
    ]));
  }
}
