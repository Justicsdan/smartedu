...
import 'pages/student_ace_dashboard.dart';
...
class _StudentDashboardState extends State<StudentDashboard> {
  ...
  bool get _isAce =>
      context.read<StudentProvider>().curriculumMode == 'ace';
  ...
  List<_SidebarItem> get _sidebarItems {
    return <_SidebarItem>[
      ...
      _isAce
          ? const _SidebarItem(
              icon: Icons.auto_stories_rounded, label: 'Progress')
          : const _SidebarItem(
              icon: Icons.bar_chart_rounded, label: 'Results'),
      const _SidebarItem(icon: Icons.assignment_rounded, label: 'Assignments'),
      const _SidebarItem(icon: Icons.quiz_rounded, label: 'CBT Exams'),
      const _SidebarItem(
          icon: Icons.campaign_rounded, label: 'Announcements'),
      const _SidebarItem(
          icon: Icons.event_available_rounded, label: 'Attendance'),
      const _SidebarItem(icon: Icons.receipt_long_rounded, label: 'Fees'),
      const _SidebarItem(
  ...
  String _getStudentName(StudentProvider provider) {
    ...
    if (data != null) {
      ...
      if (first.isNotEmpty || last.isNotEmpty) {
        return '$first ${middle.isNotEmpty ? '$middle ' : ''}$last'.trim();
      }
                ...
                builder: (context, provider, _) {
                  return Row(
                    children: [
                      ...
                      Expanded(
                        child: Text(
                          _getSchoolName(provider).isNotEmpty
                ...
                builder: (context, provider, _) {
                  ...
                  return Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        Text(
                          '${provider.currentSession!['name'] ?? 'Session'}',
                          style: const TextStyle(
                              color: Colors.white,
                              fontSize: 11,
                              fontWeight: FontWeight.w500),
                        ...
                        Text(
                          '${provider.currentTerm!['name'] ?? 'Term'}',
                          style: TextStyle(
        ...
        builder: (context, provider, _) {
          return Row(
            children: [
              ...
              if (!isSmall)
                Text(_sidebarItems[_selectedIndex].label,
                    style: const TextStyle(
                        fontSize: 20,
  ...
  Widget _buildCurrentPage() {
    switch (_selectedIndex) {
      ...
      case 1:
        return _isAce
            ? const StudentAceDashboard()
            : const StudentResultsPage();
      case 2:
        return const StudentAssignmentsPage();
      case 3:
        return const StudentCbtPage();
      case 4:
