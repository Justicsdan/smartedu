...
class _StudentAceDashboardState extends State<StudentAceDashboard> {
  ...
  Future<void> _loadData() async {
    ...
    final subjectIds = provider.myPaceScores
    ...
    if (subjectIds.isNotEmpty) {
      try {
        ...
        if (mounted) {
          setState(() {
            _subjects = List<Map<String, dynamic>>.from(result);
            _isLoading = false;
          });
        }
      } catch (e) {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          if (r['day_in_term'] != null)
            _infoRow(
                'Day in Term', '${r['day_in_term']}', Colors.white70),
          if (r['published_at'] != null)
            _infoRow(
                'Date',
                (r['published_at'] as String).split('T').first,
                Colors.white70),
        ],
      ),
    );
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              _rpField('W.P.W', r['reading_wpm']),
              const SizedBox(width: 12),
              _rpField('W.P.W Comprehension %', r['reading_comprehension']),
              const SizedBox(width: 12),
              _rpField('Composite', r['reading_composite']),
            ],
          ),
          const SizedBox(height: 10),
          Row(
            children: [
              _rpReadOnly('NO. of PACEs completed',
                  r['paces_completed']?.toString() ?? '0'),
              const SizedBox(width: 12),
              _rpReadOnly(
                  'PACE score AVG', _fmtNum(r['paces_avg'])),
              const SizedBox(width: 12),
              _rpReadOnly('Total (PACES Cass)',
                  _fmtNum(r['paces_total'])),
            ],
          ),
          const SizedBox(height: 10),
          Row(
            children: [
              _rpField(
                  ...
                  r['published_at'] != null
                      ...
                      : null),
              const SizedBox(width: 12),
              _rpField('Days in Term', r['day_in_term']),
              const SizedBox(width: 12),
              _rpField('Days Absent', r['days_absent']),
            ],
          ),
          const SizedBox(height: 12),
          _rpCommentSection(
              'Supervisor\'s Comment', r['supervisor_comment']),
          const SizedBox(height: 10),
          _rpCommentSection(
              'Principal\'s Comment', r['principal_comment']),
        ],
      ),
    );
  }
      ...
      children: [
        ...
        const Align(
          ...
          child: SizedBox(
            width: 200,
            child: Divider(
                color: Color(0xFFAAAAAA), thickness: 0.5),
          ),
        ),
        const SizedBox(height: 4),
  ...
  Widget _buildFooterCard(Map<String, dynamic> r) {
    return Container(
      ...
      child: Row(
        children: [
          _footerItem('HACS Score', _fmtNum(r['hacs_score']),
              const Color(0xFF1A237E)),
          const SizedBox(width: 12),
          _footerItem('NCE Score', _fmtNum(r['nce_score']),
              const Color(0xFFE65100)),
          const SizedBox(width: 12),
          _footerItem('Total', _fmtNum(r['total_score']),
              const Color(0xFF2E7D32)),
        ],
      ),
    );
    ...
    try {
      await AcePdfGenerator.generateAndDownload(
        ...
        student: {
          ...
          'passport_url': provider.passportUrl,
          'admission_no': provider.admissionNo,
        },
