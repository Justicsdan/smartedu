...
abstract class TeacherBase extends ChangeNotifier {
  ...
  Map<String, dynamic>? get currentTeacher => {
    'id': _teacherId, 'first_name': _firstName, 'last_name': _lastName, 'staff_id': _staffId, 'email': _email
  };

  Future<bool> login(String username, String password) async {
    try {
      final r = await Supabase.instance.client.from('teachers').select('*, schools(name, logo_url)').eq('username', username.trim()).eq('password', password).eq('is_active', true).maybeSingle();
      if (r == null) return false;
      _teacherId = r['id'].toString(); _schoolId = r['school_id'].toString();
      _firstName = r['first_name'] ?? ''; _lastName = r['last_name'] ?? '';
      _email = r['email'] ?? ''; _phone = r['phone'] ?? ''; _staffId = r['staff_id'] ?? '';
      final school = r['schools'] as Map<String, dynamic>? ?? {};
      _schoolName = school['name'] ?? ''; _schoolLogoUrl = school['logo_url'] ?? '';
  ...
  Future<void> _loadSessions() async {
    try {
      final r = await Supabase.instance.client.from('academic_sessions').select().eq('school_id', _schoolId).order('name', ascending: false);
      _academicSessions = List<Map<String, dynamic>>.from(r);
      _currentSession = _academicSessions.isNotEmpty ? _academicSessions.firstWhere((s) => s['is_current'] == true, orElse: () => _academicSessions.first) : null;
      if (_currentSession != null) await _loadTerms();
    } catch (e) { debugPrint('Load sessions error: $e'); }
  }

  Future<void> _loadTerms() async {
    if (_currentSession == null) return;
    try {
      final r = await Supabase.instance.client.from('terms').select().eq('school_id', _schoolId).eq('session_id', _currentSession!['id']).order('created_at');
      _terms = List<Map<String, dynamic>>.from(r);
      _currentTerm = _terms.isNotEmpty ? _terms.firstWhere((t) => t['is_current'] == true, orElse: () => _terms.first) : null;
    } catch (e) { debugPrint('Load terms error: $e'); }
  }

  Future<void> _loadSettings() async {
    try {
      final r = await Supabase.instance.client.from('school_settings').select().eq('school_id', _schoolId).maybeSingle();
      if (r != null) {
        _schoolSettings = Map<String, dynamic>.from(r);
        _gradingSystem = List<Map<String, dynamic>>.from(r['grading_system']?.map((e) => Map<String, dynamic>.from(e)) ?? []);
        _assessmentTypes = List<Map<String, dynamic>>.from(r['assessment_types']?.map((e) => Map<String, dynamic>.from(e)) ?? []);
        _subjectMaxScore = r['subject_max_score'] ?? 100;
        _showPosition = r['show_position'] ?? true;
      }
    } catch (e) { debugPrint('Load settings error: $e'); }
  }

  Future<bool> setCurrentTerm(String termId) async {
    try { _currentTerm = _terms.firstWhere((t) => t['id'] == termId); await loadTeacherData(); notifyListeners(); return true; } catch (e) { return false; }
  }

  String calculateGrade(double total) {
    for (var g in _gradingSystem) { if (total >= (g['min'] as num) && total <= (g['max'] as num)) return g['grade'] as String; }
    return 'F';
  }
  ...
  Future<void> initialize({
    ...
    try {
      debugPrint('Teacher initialize: schoolId=$schoolId, teacherId=$teacherId');
      _schoolId = schoolId;
      ...
      final school = await Supabase.instance.client.from('schools').select('name, logo_url').eq('id', _schoolId).maybeSingle();
  ...
  Future<void> loadMyAssignments() async {
    try {
      ...
      _myAssignments = List<Map<String, dynamic>>.from(r);
    } catch (e) {
  ...
  Future<bool> addAssignment(Map<String, dynamic> data) async {
    ...
    try {
      ...
      _myAssignments.insert(0, Map<String, dynamic>.from(r));
      notifyListeners();
      return true;
    } catch (e) {
